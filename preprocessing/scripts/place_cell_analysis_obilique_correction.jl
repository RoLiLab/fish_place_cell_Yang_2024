...
y_fish_ob, x_fish_ob, chamber_roi, countour_array = h5open(loc_roi_file) do file
    read(file, "y_fish"), 
    read(file, "x_fish"),
    read(file, "chamber_roi"),
    read(file, "countour_array")
end;

w = size(chamber_roi, 1)
l = size(chamber_roi, 2);

countour_array_x = countour_array[:,1]
countour_array_y = countour_array[:,2]

x_length = extrema(countour_array_x)[2] - extrema(countour_array_x)[1]
y_length = extrema(countour_array_y)[2] - extrema(countour_array_y)[1]

long_axis = Float32.(countour_array_x)

interval = 40
diff_long_axis = abs.(diff(long_axis[1:interval:end]));
diff_long_axis_fit = fit(TrendFilter, Float64.(diff_long_axis) , 3, 5);
diff_long_axis = diff_long_axis_fit.β;

start_duration = py"find_consecutive"(diff_long_axis .>20)
duration_all = [start_duration[i][2] for i in 1:length(start_duration)]

which_edge = findmax([mean(abs.(diff(long_axis[1:interval:end]))[start_duration[i][1]+1:start_duration[i][1]+start_duration[i][2]]) for i in 1:length(start_duration)])[2]
long_axis_index = start_duration[which_edge][1]*interval+1:(start_duration[which_edge][1]*interval+start_duration[which_edge][2]*interval)

slope, _,_,_ = stats.linregress(countour_array_x[long_axis_index], countour_array_y[long_axis_index])
what_angle = -180*numpy.arctan(slope)/pi

chamber_roi = skimage_transform.rotate(chamber_roi, what_angle);
...
mkpath(joinpath(data_path(ds_save), "oblique_corrected"))

start_min = floor(Int32,start_index/120)
end_min = floor(Int32,end_index/120)

save_file_name = "$(which_activity)_spatial_info_$(start_min)_$(end_min)_$(loc_roi_filename)_sigma$(sigma)_n$(n_pos)_A_dF_oblique_corrected.h5"
if end_p>0 && end_p<=1
    mkpath(joinpath(data_path(ds_save), "place_cell_windows"))
    save_file_name = "place_cell_windows/$(which_activity)_spatial_info_$(start_min)_$(end_min)_$(loc_roi_filename)_sigma$(sigma)_n$(n_pos)_A_dF_oblique_corrected.h5"
end
...
h5open(joinpath(data_path(ds_save),"oblique_corrected", "for_place_calculation_$(loc_roi_filename)_n$(n_pos)_oblique_corrected.h5"), "w") do file
...
for neuron_idx in valid_neurons
    ...
    spatial_info_all[neuron_idx], dF_mean_all[neuron_idx] = MAP.spatial_info(cur_map, mask_map)
...
h5open(joinpath(data_path(ds_save),"oblique_corrected", save_file_name), "w") do file
...
#     for neuron_idx in candidate_place_cell
#     # for neuron_idx in valid_neurons
#         neural_activity = A_dFF[:,neuron_idx]
#         neural_activity, which_loc = MAP.valid_activity_loc(neural_activity, bool_index,loc_digital)
#         if length(neural_activity)<at_least_frame
...
#     place_cell_index = intersect(findall(specificity_population_z.>=3), findall(specificity_shuffle_z.>=5));
#     println("Number of place cells: ", length(place_cell_index))

#     h5open(joinpath(data_path(ds_save), save_file_name), "r+") do file
